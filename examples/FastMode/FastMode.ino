...
setup ()
{
  ...
  /* Initialize Hourglass library with fast cycles (time per tick: 1 ms)  */
  Hourglass.Init (FAST);


  /* After initializion, the library behaves exactly as it did in standard
     (e.g. SLOW) mode, but tick time is reduced to 1 ms. This produces more
     load for the CPU. It is recommended to not use FAST mode as long as the
     program can also be written using standard (SLOW) mode  */


  /* Register event
     0:          Time (in Ticks, e.g. 0 x 1 ms) to first execution
     ToggleLed:  Function to execute
     500:        Cycle time (in Ticks, e.g. 500 x 1 ms)
     ...
     on pin 13  */
  Hourglass.RegisterEvent (0, ToggleLed, 500);
}
